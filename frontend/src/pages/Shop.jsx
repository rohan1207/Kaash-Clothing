...
const Shop = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (filters.price !== 'All') {
      items = items.filter(p => {
        ...
        if (filters.price === '1000 - 2000') return price >= 1000 && price <= 2000;
        if (filters.price === '2000 - 5000') return price >= 2000 && price <= 5000;
  ...
  const FilterSidebar = () => (
    ...
    >
      <div className="flex justify-between items-center mb-12">
        <h2 className="text-3xl font-serif text-gray-800">Refine Your Search</h2>
        <button onClick={() => setIsFilterOpen(false)} className="text-gray-500 hover:text-gray-900 transition-transform duration-300 hover:rotate-90">
          <FiX size={28} />
        </button>
      </div>
      ...
      <div className="space-y-10">
          ...
          <div className="space-y-3">
            {categories.map(cat => (
              <button
                ...
                className={`w-full text-left p-3 rounded-lg transition-all duration-300 text-lg ${filters.category === cat ? 'bg-gray-800 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div>
          <h3 className="text-xl font-semibold text-gray-700 mb-5">Price Range</h3>
          <div className="space-y-3">
            {priceRanges.map(range => (
              <button
                key={range}
                onClick={() => handleFilterChange('price', range)}
                className={`w-full text-left p-3 rounded-lg transition-all duration-300 text-lg ${filters.price === range ? 'bg-gray-800 text-white shadow-md' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {range}
              </button>
            ))}
          </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      ...
      {isFilterOpen && <div className="fixed inset-0 bg-black/60 z-40 backdrop-blur-sm" onClick={() => setIsFilterOpen(false)}></div>}

      <main className="pt-28 md:pt-36 pb-24">
        <header className="text-center mb-16 px-6">
          <h1 className="text-5xl md:text-7xl text-gray-800 font-serif tracking-tight mb-4">Curated Collection</h1>
          <p className="text-gray-600 max-w-3xl mx-auto text-lg">
            Experience the pinnacle of craftsmanship and style. Each piece is a statement of elegance, designed for the modern connoisseur.
          </p>
        </header>
        ...
        <div className="container mx-auto px-6">
          <div className="mb-10 flex justify-between items-center">
            ...
            <p className="text-md text-gray-500 font-medium">{filteredAndSortedProducts.length} results</p>
          </div>
